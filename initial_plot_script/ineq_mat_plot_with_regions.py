...
def main():
    ...
    reg_df = pd.read_csv("state_regions.csv")
    all_merge_df = all_df.merge(reg_df, on="st")
    reg_grouped_df = all_merge_df.groupby(["Year", "Region"]).mean().drop(columns="st")
    for reg in regions:
        j = 0
        i = 0
        x = []
        y = []
        for i in reg_grouped_df["Gini"]:
            if (reg_grouped_df.index[j][1] == reg):
                x.append(reg_grouped_df.index[j][0])
                y.append(i)
            j += 1
        plt.plot(x, y)
    line_plot(all_df, states["US"])
    plt.show()
    return 0
